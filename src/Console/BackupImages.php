...
class BackupImages extends AbstractCommand
{
    ...
    private array $brokenHosts = [
        ...
        'directupload.net',
        'forum.archlinux.de', // 3 images in ~3 posts, old troubleshooting threads; not worth recovering
        'imagebanana.com',
        'imageshack.com',
        'imageshack.se',
        'imageshack.us',
        ...
        'ompldr.org',
        'omploader.org',
        'paste.archlinux.de', // 7 images in ~7 posts, not on archive.org; not worth recovering
        'picfront.org',
        'tinypic.com',
    ];
    ...
    protected function fire(): void
    {
        ...
        $failedFirstPass = $this->attemptDirectDownloads($posts); // @phpstan-ignore argument.type
        ...
        $archiveUrlsToDownload = $this->findUrlsInArchive($failedFirstPass);
        ...
        $this->downloadFromArchive($archiveUrlsToDownload);
     ...
     */
    private function attemptDirectDownloads(Collection $posts): array
    {
        $this->info("---" . "Stage 1: Attempting direct downloads" . "---");
        $failedFirstPass = [];

        $failedUrls = file_exists($this->failedLog)
            ? file($this->failedLog, FILE_SKIP_EMPTY_LINES | FILE_IGNORE_NEW_LINES)
            : [];
        ...
        $requests = function () use ($posts, $failedUrls) {
            ...
            foreach ($posts as $post) {
                ...
                foreach ($matches[1] as $url) {
                    ...
                    if (in_array($url, $failedUrls, true)) {
                        continue;
                    }
        ...
        $pool = new Pool($this->httpClient, $requests(), [
            ...
            'rejected' => function (\Throwable $e, $index) use (&$failedFirstPass): void {
                assert(is_array($index) && isset($index['url']) && isset($index['host']));
                assert(is_string($index['url']) && is_string($index['host']));
                if ($this->isBrokenHost($index['host'])) {
                    $failedFirstPass[] = $index;
                    $this->info("Direct download failed, will retry via archive.org: " . $index['url']);
                } else {
                    $this->logFailure($index['url'], 'download failed');
                }
            },
        ]);
     ...
     */
    private function findUrlsInArchive(array $failedFirstPass): array
    {
        ...
        $pool = new Pool($this->httpClient, $requests(), [
            ...
            'fulfilled' => function (ResponseInterface $response, $index) use (&$archiveUrlsToDownload): void {
                ...
                try {
                    ...
                    $finalUrl = null;
                    $closest = $json['archived_snapshots']['closest'] ?? null;
                    if (is_array($closest)) {
                        $finalUrl = $closest['url'] ?? null;
                    }
     ...
     */
    private function downloadFromArchive(array $archiveUrlsToDownload): void
    {
        $count = count($archiveUrlsToDownload);
        $this->info("---Stage 3: Downloading $count images found in archive.org---");
    ...
    private function saveImage(ResponseInterface $response, string $url, string $filepath): void
    {
        ...
        if ($lastModified && count($lastModified) == 1) {
            $timestamp = strtotime($lastModified[0]);
            if ($timestamp !== false) {
